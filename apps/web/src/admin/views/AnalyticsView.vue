<template>
  <div v-if="data">
    ...
    <el-row :gutter="16">
      ...
      <el-col :xs="12" :sm="8" :md="4">
        <el-card class="stat"><span class="s-label">Gem. waarde</span><span class="s-val">{{ site.formatMoney(data.avgValueCents) }}</span></el-card>
      </el-col>
      <el-col :xs="12" :sm="8" :md="4">
        <el-card class="stat"><span class="s-label">Reserveringen</span><span class="s-val">{{ data.totalBookings }}</span></el-card>
      </el-col>
      <el-col :xs="12" :sm="8" :md="4">
        <el-card class="stat"><span class="s-label">Afgerond</span><span class="s-val">{{ data.completed }}</span></el-card>
      </el-col>
      <el-col :xs="12" :sm="8" :md="4">
...
// Tiny inline ranking list component (count bar + revenue).
const Ranking = (props: { rows: { name: string; count: number; revenueCents: number }[]; empty: string }) => {
  ...
  return h(
    ...
    props.rows.map((r) =>
      h('div', { class: 'rrow', key: r.name }, [
        ...
        h('span', { class: 'rrev' }, site.formatMoney(r.revenueCents)),
      ]),
    ),
  );
};
...
:deep(.rname) { width: 130px; text-align: right; color: var(--el-text-color-secondary); font-size: 0.9rem; }
...
:deep(.rrev) { width: 80px; text-align: right; font-weight: 600; font-size: 0.85rem; }
